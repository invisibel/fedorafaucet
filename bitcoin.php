...
define("BITCOIN_ADDRESS_VERSION", "7D");// this is a hex byte
/**
...
require_once(dirname(__FILE__) . "/includes/xmlrpc.php");
require_once(dirname(__FILE__) . "/includes/jsonrpc.php");
...
class BitcoinClient extends jsonrpc_client {
    ...
    {
        return $this->decoderawtransaction($this->query("getrawtransaction", $txid));
    }
    public function decoderawtransaction($raw)
    {
        return $this->query("decoderawtransaction", $raw);
    }
    public function getTransactionSender($txid)
    {
        $txdetails = $this->getrawtransaction($txid, (int)1);
        foreach ($txdetails['vin'] as $vin)
        {
            $script = explode(" ", $vin['scriptSig']['asm']);
            return Bitcoin::pubKeyToAddress($script[1]);
        }
    }
